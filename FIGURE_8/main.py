...
arg = [
       [(Pplus1)/(L-h1), 0, epsilon, Pplus1],
       [ (alpha1/beta1)*(Pplus1)/(L-h1),  0 ,epsilon, (alpha1/beta1)*Pplus1],
       [(-Pplus2)/h2, Pplus2, epsilon, Pplus2],
       [(alpha2/beta2)*(-Pplus2)/h2, (alpha2/beta2)*Pplus2, epsilon, Pplus2]
       ]
...
log_norm = LogNorm(vmin=1e-2, vmax=Pplus/2)
...
for i in range(len(Time_to_plot)):
    ...
    ax[f'ax_u1{i}'].imshow(np.clip(grid_u1, 1e-2, Pplus/2), origin='lower',
    ...
    ax[f'ax_u2{i}'].imshow(np.clip(grid_u2, 1e-2, Pplus/2), origin='lower',
                   extent=[grid_x.min(), grid_x.max(), grid_y.min(), grid_y.max()],
...
fig.text(
    ...
    s="Simulation of the scenario with hardwood enclaves in the boreal forest",
    ha='center', va='bottom',
...
fig.savefig('output/scenario2_with_fire.pdf', format='pdf', dpi=300, transparent=True, bbox_inches='tight', pad_inches=0.0)
